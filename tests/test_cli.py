...
class HygieneApprovalQueueCliTest(unittest.TestCase):
    def test_accepts_exact_run_id_selector_with_dry_run(self) -> None:
        ...
        self.assertIn("Hygiene Approval Queue Dry Run Preview:", output)
        self.assertIn("- stage: dry-run", output)
    ...
    def test_accepts_exact_run_id_selector_with_apply(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            approval_path = _write_minimal_pending_approval(root, run_id="RUN-20260327T055614Z")
            stdout = StringIO()
            ...
            self.assertEqual(exit_code, 0)
            output = stdout.getvalue()
            self.assertIn("Hygiene Approval Queue Apply:", output)
            ...
            self.assertEqual(payload["queue_hygiene"]["requested_approval_id"], "APR-RUN-20260327T055614Z")

    def test_accepts_exact_approval_id_selector_with_apply(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            approval_path = _write_minimal_pending_approval(root, run_id="RUN-20260327T055614Z")
            stdout = StringIO()
    ...
    def test_refuses_when_mode_is_missing(self) -> None:
        stderr = StringIO()

        with self.assertRaises(SystemExit) as exc_info, redirect_stderr(stderr):
            main(["hygiene-approval-queue", "--root", ".", "--run-id", "RUN-20260327T055614Z"])

        self.assertEqual(exc_info.exception.code, 2)
        self.assertIn("one of the arguments --dry-run --apply is required", stderr.getvalue())

    def test_refuses_when_selector_is_missing(self) -> None:
        stderr = StringIO()

        with self.assertRaises(SystemExit) as exc_info, redirect_stderr(stderr):
            main(["hygiene-approval-queue", "--root", ".", "--dry-run"])
    ...
    def test_refuses_when_both_modes_are_provided(self) -> None:
        stderr = StringIO()
    ...
    def test_refuses_malformed_run_id(self) -> None:
        ...
        with self.assertRaises(SystemExit) as exc_info, redirect_stderr(stderr):
            main(["hygiene-approval-queue", "--root", ".", "--run-id", "RUN_20260327", "--dry-run"])
    ...
    def test_refuses_malformed_approval_id(self) -> None:
        ...
        with self.assertRaises(SystemExit) as exc_info, redirect_stderr(stderr):
            main(["hygiene-approval-queue", "--root", ".", "--approval-id", "APR_RUN_20260327", "--dry-run"])
    ...
    def test_refuses_heuristic_pseudo_run_id(self) -> None:
        for value in ("stale", "latest", "RUN-stale"):
            ...
            with self.assertRaises(SystemExit) as exc_info, redirect_stderr(stderr):
                main(["hygiene-approval-queue", "--root", ".", "--run-id", value, "--dry-run"])
    ...
    def test_refuses_heuristic_pseudo_approval_id(self) -> None:
        for value in ("APR-latest", "APR-stale", "latest"):
            ...
            with self.assertRaises(SystemExit) as exc_info, redirect_stderr(stderr):
                main(["hygiene-approval-queue", "--root", ".", "--approval-id", value, "--dry-run"])

            self.assertEqual(exc_info.exception.code, 2)
            self.assertIn("heuristic or pseudo selectors such as stale/latest are not allowed", stderr.getvalue())

    def test_refuses_when_no_matching_queue_artifact_exists(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            stderr = StringIO()
        ...
        self.assertEqual(exc_info.exception.code, 2)
        self.assertIn("found no matching queue artifact under approval_queue/pending/", stderr.getvalue())

    def test_refuses_when_multiple_matching_queue_artifacts_exist(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_minimal_pending_approval(root, run_id="RUN-20260327T055614Z")
            duplicate = root / "approval_queue" / "pending" / "APR-RUN-20260327T055614Z--r2.yaml"
            duplicate.write_text(
                "\n".join(
                    [
                        "approval_request:",
                        "  id: APR-RUN-20260327T055614Z",
                        "  status: pending",
                    ]
                )
                + "\n",
        ...
        self.assertEqual(exc_info.exception.code, 2)
        self.assertIn("found multiple matching queue artifacts under approval_queue/pending/", stderr.getvalue())

    def test_dry_run_execution_one_shot_and_no_mutation(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_minimal_pending_approval(root, run_id="RUN-20260327T055614Z")
            ...
            with self.assertRaises(SystemExit) as exc_info, redirect_stderr(heuristic_stderr):
                main(["hygiene-approval-queue", "--root", str(root), "--run-id", "RUN-stale", "--dry-run"])

            self.assertEqual(exc_info.exception.code, 2)
            self.assertEqual(before, _snapshot_files(root))

            missing_stderr = StringIO()
            with self.assertRaises(SystemExit) as exc_info, redirect_stderr(missing_stderr):
                main(
                    [
                        "hygiene-approval-queue",
                        "--root",
                        str(root),
                        "--approval-id",
                        "APR-RUN-20260327T099999Z",
                        "--dry-run",
                    ]
                )

            self.assertEqual(exc_info.exception.code, 2)
            self.assertIn("found no matching queue artifact under approval_queue/pending/", missing_stderr.getvalue())
            self.assertEqual(before, _snapshot_files(root))

    def test_apply_execution_one_shot_and_exact_target_only_mutation(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            target_path = _write_minimal_pending_approval(root, run_id="RUN-20260327T055614Z")
            other_path = _write_minimal_pending_approval(root, run_id="RUN-20260327T063724Z")
            before = _snapshot_files(root)

            run_dry_run_stdout = StringIO()
            with redirect_stdout(run_dry_run_stdout):
                exit_code = main(
                    [
                        "hygiene-approval-queue",
                        "--root",
            ...
            run_apply_stdout = StringIO()
            with redirect_stdout(run_apply_stdout):
                exit_code = main(
                    [
                        "hygiene-approval-queue",
                        "--root",
            ...
            self.assertEqual(changed_after_run_apply, ["approval_queue/pending/APR-RUN-20260327T055614Z.yaml"])

            approval_apply_stdout = StringIO()
            with redirect_stdout(approval_apply_stdout):
                exit_code = main(
                    [
                        "hygiene-approval-queue",
                        "--root",
            ...
            self.assertEqual(target_payload["queue_hygiene"]["status"], "applied")
            self.assertEqual(target_payload["queue_hygiene"]["requested_run_id"], "RUN-20260327T055614Z")
            self.assertEqual(target_payload["queue_hygiene"]["requested_approval_id"], "APR-RUN-20260327T055614Z")

            other_payload = read_yaml(other_path)
            self.assertEqual(other_payload, read_yaml(other_path))
            self.assertNotIn("queue_hygiene", other_payload)

            conflicting_selector_stderr = StringIO()
            with self.assertRaises(SystemExit) as exc_info, redirect_stderr(conflicting_selector_stderr):
                main(
                    [
                        "hygiene-approval-queue",
                        "--root",
            ...
            conflicting_mode_stderr = StringIO()
            with self.assertRaises(SystemExit) as exc_info, redirect_stderr(conflicting_mode_stderr):
                main(
                    [
                        "hygiene-approval-queue",
                        "--root",
            ...
            heuristic_stderr = StringIO()
            with self.assertRaises(SystemExit) as exc_info, redirect_stderr(heuristic_stderr):
                main(["hygiene-approval-queue", "--root", str(root), "--run-id", "RUN-stale", "--apply"])
            ...
            self.assertEqual(exc_info.exception.code, 2)
            self.assertIn("found no matching queue artifact under approval_queue/pending/", missing_stderr.getvalue())
            self.assertNotIn("queue_hygiene", read_yaml(other_path))
...
class CleanupRehearsalCliTest(unittest.TestCase):
    ...
    def test_inspect_approval_queue_shows_latest_and_stale_pending_items(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("queue_hygiene_audit:", output)

    def test_inspect_approval_queue_shows_item_local_queue_hygiene_audit_only_when_metadata_exists(self) -> None:
        from pathlib import Path
    ...
    def test_status_does_not_surface_queue_hygiene_visibility(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            pending = root / "approval_queue" / "pending"
            pending.mkdir(parents=True, exist_ok=True)
            (pending / "APR-RUN-405.yaml").write_text(
                "\n".join(
                    [
                        "approval_request:",
                        "  id: APR-RUN-405",
            ...
            self.assertIn("Approval Queue:", output)
            self.assertIn("- pending_total: 2", output)
            self.assertNotIn("queue_hygiene", output)
            self.assertNotIn("selector_family", output)
            self.assertNotIn("requested_run_id", output)
...
class LatestRunCliTest(unittest.TestCase):
    ...
    def test_record_review_success_output_guides_to_inspect_run_with_resolved_run_id(self) -> None:
        from pathlib import Path
        ...
        with TemporaryDirectory() as tmp:
            ...
            with redirect_stdout(stdout):
                exit_code = main(
                    ["record-review", "--root", str(root), "--run-id", "RUN-NEW", "--status", "pass", "--summary", "review ok"]
                )

            self.assertEqual(exit_code, 0)
            output = stdout.getvalue()
            self.assertIn("Review Recorded:", output)
            self.assertIn("- run_id: RUN-NEW", output)
            self.assertIn(
                f"- review_report_path: {(root / 'runs' / 'latest' / 'RUN-NEW' / 'artifacts' / 'review-report.yaml').as_posix()}",
                output,
            )
            self.assertIn("- next: factory inspect-run --root . --run-id RUN-NEW", output)

    def test_record_qa_success_output_guides_to_inspect_run_with_resolved_run_id(self) -> None:
        from pathlib import Path
        ...
        with TemporaryDirectory() as tmp:
            ...
            with redirect_stdout(stdout):
                exit_code = main(
                    ["record-qa", "--root", str(root), "--run-id", "RUN-NEW", "--status", "pass", "--summary", "qa ok"]
                )

            self.assertEqual(exit_code, 0)
            output = stdout.getvalue()
            self.assertIn("QA Recorded:", output)
            self.assertIn("- run_id: RUN-NEW", output)
            self.assertIn(
                f"- qa_report_path: {(root / 'runs' / 'latest' / 'RUN-NEW' / 'artifacts' / 'qa-report.yaml').as_posix()}",
                output,
            )
            self.assertIn("- next: factory inspect-run --root . --run-id RUN-NEW", output)
    ...
    def test_record_verification_success_output_guides_to_inspect_run_with_resolved_run_id(self) -> None:
        from pathlib import Path
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(exit_code, 0)
            output = stdout.getvalue()
            self.assertIn("Verification Recorded:", output)
            self.assertIn("- run_id: RUN-NEW", output)
            self.assertIn(
                f"- verification_report_path: {(root / 'runs' / 'latest' / 'RUN-NEW' / 'artifacts' / 'verification-report.yaml').as_posix()}",
                output,
            )
            self.assertIn("- next: factory inspect-run --root . --run-id RUN-NEW", output)
